...
Arrays aus Objekten werden als Tabellen interpretiert. Der Zeitstrahl verwendet `Date` (YYYY-MM-DD) und die in den Einstellungen gewählten Start-/Endzeit-Felder (HH:mm). Übliche Namen wie `Start`, `start_time`, `Startzeit`, `Beginn`, `beginning`, `End`, `Endzeit`, `Ende`, `stop` und `finish` werden automatisch erkannt, unabhängig von Groß-/Kleinschreibung sowie Leerzeichen, Unterstrichen und Bindestrichen. Ohne Treffer bleibt die jeweilige Zuordnung leer und muss manuell vorgenommen werden. `Start` und `End` bleiben gültige Standardnamen. Beliebige weitere Felder sind suchbar und als Spalten auswählbar. Verschachtelte Objekte und Arrays bleiben unverändert erhalten. Die Tabelle zeigt zunächst skalare Spalten; weitere Felder lassen sich über Anzeige einblenden. Ein Klick auf eine Tabellenzeile (oder Enter/Leertaste) öffnet den vollständigen Datensatz als aufklappbare Hierarchie, unabhängig von sichtbaren Spalten. Hat die Zeile eine EventID, wird zusätzlich ein exakter EventID-Filter gesetzt. Auch ein Klick auf den Zeitstrahl setzt diesen Filter; der Suchtext bleibt unverändert. Ein bestehender EventID-Filter wird ersetzt, andere Filter bleiben erhalten. In der Detailansicht legt ein Rechtsklick (oder Umschalt+F10/Menütaste, Escape schließt nur das Menü) auf einen Wert oder einen Ordnerknoten einen Filter an: Skalare filtern exakt und typgenau am exakten Pfad (inklusive `null`, leerem Text, `false` und `0`), Ordner prüfen nur die Existenz dieses Pfads. Gleiche Feldnamen in verschiedenen Zweigen, punkt-/slash-haltige Schlüssel und Array-Positionen werden über den angezeigten Pfad unterschieden; doppelte Filter werden nicht erneut angelegt. Chips zeigen Pfad und Wert/Existenz und lassen sich einzeln entfernen; nach dem Anlegen schließt die Detailansicht, damit die gefilterte Tabelle sichtbar wird. Bestehende Enthält-/Gleich-Filter und gespeicherte Ansichten behalten ihre Semantik.

Der Zeitstrahl lässt sich über das Fenster-Symbol rechts oben in ein separates Browserfenster lösen (Popup mit eigener Größe, angefragter Fensterstil kann je nach Browser als Tab öffnen). Dort bleiben Datum, Filter, Quelle, Seitennavigation und Legende interaktiv und synchron; inline erscheint ein Platzhalter. Schließen des Fensters bettet den Zeitstrahl wieder ein und löscht die gespeicherte Ablösung, Schließen/Neuladen der App behält sie und räumt nur verwaiste Fenster auf. Wird das Popup blockiert, bleibt der Zeitstrahl inline nutzbar und eine Wiederherstellungsaktion öffnet ihn per Klick erneut. Es wird nie ein zweiter Speicher/OPFS-Schreiber im Kindfenster erzeugt. Beispiel:
